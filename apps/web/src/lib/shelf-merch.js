...
================================================================= */
  ...
  const defaultState = {
    ...
    loading: false,
    account: "Rubix",
    user: { name: "Chandra Sekhar", initials: "CS", email: "hr@rubix.net" },
    catalogProducts: [],
    catalogTotal: 0,
    campaigns: [],
    ...
    wallets: [
      ...
      },
      {
        ...
        activity: [
          { t: "Funds added", who: "Purchase order #PO-3391", amt: 142000, on: "30 May 2026" },
        ],
      },
      {
        id: "w3",
  ...
  function go(view, opts = {}) {
    S.view = view;
    if (opts.nav) S.nav = opts.nav;
    Object.assign(S.flow, opts.flow || {});
    if (view === "contacts") S.flow.contactsSearch = "";
    window.scrollTo(0, 0);
    render();
  }
  ...
  function render() {
    syncAuthState();
    if (S.loading) {
      APP().innerHTML = `<div class="auth"><div style="display:grid;place-items:center;min-height:60vh;color:var(--ink-2);font-size:15px">Loading…</div></div>`;
      return;
    }
    if (!S.authed) {
  ...
  Wizards.editKit = function () {
    ...
    const back =
      step > 0
        ? backLink("Back", "ekBack", null, { mb: "0" })
        : backLink("Cancel", "wzExit", null, { mb: "0" });
    const next =
      step < 1
  ...
  async function kitSaveEdit() {
    const f = S.flow;
    if ((f.step | 0) === 0)
      f.kitName = (document.getElementById("ek-name") || {}).value || f.kitName;
    else f.notes = (document.getElementById("kt-notes") || {}).value || "";
    if (!f.picked.length) {
      toast("Add at least one product to the kit", false);
      return;
    }
  ...
  async function swGenerate() {
    ...
    if (api.useMocks()) {
      const col = {
        ...
        preferredColors: [],
        artworkUrl: artUrl,
        artPlacements: placements,
        products: f.picked.map((i, idx) => {
          const cp = catalog[i];
          return {
            id: cp?.id,
            g: cp?.g || "tee",
            brand: cp?.brand || "",
            nm: cp?.nm || "Product",
            printAreas: cp?.printAreas,
            imgUrl: cp?.imgUrl,
            maskImageUrl: cp?.maskImageUrl,
    ...
    try {
      ...
      const col = await api.createCollectionFlow({
        ...
        catalog,
        artwork: f.artFile
          ? { file: f.artFile.file, preview: f.artFile.preview, name: f.artFile.name }
          : undefined,
        mockups: f.picked
          .map((i, idx) => {
            const cp = catalog[i];
  ...
  Wizards.sendPoints = function () {
    ...
    const next =
      step < 3 ? `<button class="btn btn-dark" data-act="spNext">Next</button>` : "<span></span>";
  ...
  function spRecalc() {
    const ppr = +document.getElementById("sp-ppr").value || 0;
    document.getElementById("sp-pts").value = (ppr / PT).toFixed(2);
    S.flow.ppr = ppr;
    S.flow.recips = +document.getElementById("sp-recips").value || 0;
  }
  function spMsg(e) {
    S.flow.msg = e.target ? e.target.value : "";
    const p = document.getElementById("sp-prev");
    if (p) p.textContent = S.flow.msg;
  }
  function spWhen(el) {
    S.flow.when = el.dataset.arg;
    render();
  }
  function spBack() {
    if (S.flow.step === 0) {
  ...
  function spNext() {
    const f = S.flow;
    if (f.step === 0) {
      f.ppr = +document.getElementById("sp-ppr").value || 0;
      f.recips = +document.getElementById("sp-recips").value || 0;
      f.orderName = document.getElementById("sp-name").value;
    }
    if (f.step === 2) {
      const fr = document.getElementById("re-from");
      if (fr) f.from = fr.value;
  ...
  async function sendPointsDo() {
    ...
    const finishUi = (g) => {
      ...
      go("shopDetail", {
        flow: { shopId: f.shopId, shopTab: "Sent Gifts", sentGifts: [g] },
        nav: "shops",
      });
    ...
    if (!f.shopId) {
      toast("Select a shop for this campaign");
      return;
    }
    try {
      S.loading = true;
  ...
  function shopEditLogoUpload() {
    document.getElementById("shop-edit-logo-inp")?.click();
  }
  ...
  function catToggle(el) {
    ...
    S.flow.cats = S.flow.cats || ["Food & Beverages", "Work Essentials", "Merch"];
    const i = S.flow.cats.indexOf(c);
    if (i < 0) S.flow.cats.push(c);
    else S.flow.cats.splice(i, 1);
    render();
  }
  async function shopPublish() {
    ...
    if (!name) {
      toast("Enter a shop name", false);
      go("createShop", { flow: { step: 0 } });
      return;
    }
    const currency = S.flow.shopCur || "Points",
      categories = S.flow.cats || ["Food & Beverages", "Work Essentials", "Merch"];
  ...
  function shopTabBody(s, tab) {
    ...
    if (tab === "Sent Gifts") {
      ...
      <tbody>${
        (S.flow.sentGifts || []).length
          ? S.flow.sentGifts
              .map(
                (g) =>
                  `<tr><td style="font-weight:600">${esc(g.name)}</td><td class="muted">${esc(g.by)}</td><td class="num">${pts(g.ppr)}</td><td><span class="tag tag-live"><span class="dot"></span>Sent</span></td><td>${g.recips}</td><td style="text-align:right"><span class="lnk">View</span></td></tr>`,
              )
              .join("")
          : `<tr><td style="font-weight:600">Order R212828590</td><td class="muted">Jonna Madhavi</td><td class="num">742.21 Pts</td><td><span class="tag tag-warn"><span class="dot"></span>Incomplete</span></td><td>1 <span class="mut3">(99 seats left)</span></td><td style="text-align:right"><span class="lnk" data-act="sendPointsStart" data-arg="'+s.id+'">Finish sending</span></td></tr>`
      }</tbody></table>
  ...
  function swagEmptyDesigner() {
    return `<div class="card" style="padding:10px;text-align:center;max-width:720px;margin:20px auto;box-shadow:var(--sh-1)">
    <div style="position:relative;display:inline-block;max-width:100%">
      <img src="${startDesigningImg}" alt="Start designing" style="max-height:270px;max-width:100%;object-fit:contain;display:block">
      <button class="btn btn-dark btn-lg" style="position:absolute;bottom:8px;left:48px;padding:0 20px" data-act="swagDesignerStart">${I.plus}Start designing</button>
    </div>
  </div>`;
  }
  ...
  async function swagAddToShopDo() {
    ...
    try {
      ...
      toast(
        err.message ||
          (collectionMode ? "Failed to add collection to shop" : "Failed to add product to shop"),
        false,
      );
    }
  }
  ...
  function swagProductGrid(cols) {
    ...
    if (!entries.length)
      return `<div class="card empty"><h3>No products yet</h3><p>Start designing to add branded products to your swag.</p></div>`;
  ...
  function swagCollectionView(cols) {
    if (!cols.length)
      return `<div class="card empty"><h3>No collections yet</h3><p>Create a collection to group your branded products together.</p></div>`;
  ...
  function ViewCatalog() {
    ...
    const catalog = getCatalogList();
    const list =
      api.useMocks() && t !== "All Products"
        ? catalog.filter((p) => productCategory(p) === t)
        : catalog;
  ...
  async function auth() {
    ...
    try {
      ...
      toast(
        isSignup
          ? "Welcome to Shelf Merch, " + user.name.split(" ")[0] + "!"
          : "Welcome back, " + user.name.split(" ")[0],
      );
    } catch (err) {
      if (gen === bootState.gen)
  ...
  const ACT = {
    ...
    copyInvite: (el, a) => {
      navigator.clipboard
        ...
        .catch(() => toast("Copy failed — select the link manually", false));
    },
    // shops
