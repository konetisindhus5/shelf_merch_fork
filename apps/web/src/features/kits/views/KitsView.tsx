...
/** Kits dashboard: hero, stats, kit list with paging, and the insights sidebar. */
export function KitsView(vm: KitsVm) {
  ...
  const { stats, canCreateKits, canSendKits, showAll, previewLimit } = vm;
        ...
        <div className="kits-dashboard__main">
          ...
          <div className="kits-list-card">
            <div className="kits-list-toolbar">
              <div className="kits-tabs" aria-label="Kit filters">
                <button type="button" className="kits-tab kits-tab--active">
                  All kits <span>{stats.total}</span>
                </button>
                <button type="button" className="kits-tab">
                  Live <span>{stats.live}</span>
                </button>
                <button type="button" className="kits-tab">
                  Drafts <span>{stats.drafts}</span>
                </button>
                <button type="button" className="kits-tab">
              ...
              {rows.map((row) => (
                <div className="kits-table__row" role="row" key={row.id}>
                  ...
                  <div className="kits-row-actions">
                    {row.kit ? (
                      <Link to={`/app/kits/${row.id}`} className="kits-row-btn">
                        View
                      </Link>
                    ) : (
            ...
            <div className="kits-list-footer">
              <span>
                {showAll
                  ? `Showing all ${vm.kits.length} kit${vm.kits.length === 1 ? "" : "s"}`
                  : `Showing 1-${Math.min(previewLimit, rows.length)} of ${stats.total} kits`}
              </span>
              {vm.hasMoreKits && !showAll ? (
                <button type="button" className="kits-view-all" onClick={() => vm.onShowAll(true)}>
                  View all kits <ArrowRight size={17} />
                </button>
              ) : showAll && vm.hasMoreKits ? (
